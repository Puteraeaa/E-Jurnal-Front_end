...
const Leads = () => {
  ...
  const fetchData = async (pageNumber = 1, search = "") => {
    try {
      const response = await Api.get(
        `admin/parent?page=${pageNumber}&search=${search}`,
        {
          headers: {
  ...
  const getStatusClass = (lead) => {
    if (lead.industries)
      return "badge badge-secondary bg-green-500 border-green-500 w-20"; // Status is "Sedang"
  ...
  const renderPagination = () => {
    ...
    if (pagination.currentPage > 3) {
      pages.push(
        <button key="prev-ellipsis" className="join-item btn btn-disabled mr-1">
          ...
        </button>
      );
    }
      ...
      pages.push(
        <button
          ...
          className={`join-item btn ${
            pagination.currentPage === totalPages ? "btn-active" : ""
          }`}
  ...
  const filteredLeads = orangtua.filter(
    (lead) =>
  ...
  return (
    <>
      <TitleCard
        title={`Data Orang Tua (${pagination.total})`}
        topMargin="mt-2"
        TopSideButtons={
                ...
                : orangtua.map((lead) => (
                    <tr key={lead.id}>
                      ...
                      <td className="flex justify-center space-x-2 sm:space-x-4">
                        ...
                        <Link
                          to={`/app/data/orangtua/edit/${encryptId(lead.user_id)}`}
                        >
                        ...
                        <button
                          ...
                          onClick={() => handleDelete(lead.user_id)}
                        >
