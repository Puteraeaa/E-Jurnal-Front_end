...
const Leads = () => {
  ...
  const fetchData = async (pageNumber = 1, search = "") => {
    try {
      const response = await Api.get(
        `admin/industri?page=${pageNumber}&search=${search}`,
        {
          headers: {
  ...
  return (
    <>
      <TitleCard
        title={`Data Industri (${pagination.total})`} 
        topMargin="mt-2"
        TopSideButtons={
              ...
              {siswa.map((lead) => (
                <tr key={lead.id}>
                  ...
                  <td className="flex justify-center space-x-2 sm:space-x-4">
                    <Link to={`/app/data/industri/edit/${encryptId(lead.id)}`}>
      ...
      <dialog id="my_modal_5" className="modal">
        <form method="dialog" className="modal-box">
          ...
          <a
            className="btn bg-red-500 text-white w-[150px] text-xs mb-3"
            href="https://drive.google.com/drive/folders/1heRByiuCHQ7YAxJE8Azr9NMajwF0MKzT?usp=sharing"
            target="_blank"
          >
            Download Template
          </a>
