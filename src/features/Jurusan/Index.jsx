...
export default function AcademicProgramList() {
    ...
    const updateProgram = async (e) => {
        ...
        try {
            ...
            if (result.isConfirmed) {
                await Api.put(`admin/departemen/${selectedProgram.id}`, {
    ...
    const addProgram = async (e) => {
        ...
        try {
            ...
            if (result.isConfirmed) {
                await Api.post('admin/departemen', {
                    name: newProgramName,
                }, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                ...
                <div className="md:col-span-2">
                    <div className="bg-white dark:bg-[#1c2229] shadow-md rounded-lg p-6">
                        ...
                        <p className="text-gray-600 font-semibold mt-2">
                            Jumlah kejuruan yang terdftar di SMK Negeri 1 Ciomas sekarang adalah <span className="font-bold">{pagination.total}</span> kejuruan.
                        </p>
                    </div>
                </div>
                            ...
                            <table className="table-auto w-[140%] md:w-full">
                                <thead>
                                    <tr className="bg-[#3b82f5] dark:bg-gray-700 text-black dark:text-white text-sm uppercase font-semibold rounded-lg">
                                        ...
                                        <th className="p-2 text-center spacing-1 col-6">Program Name</th>
                                        ...
                                        academicPrograms.length > 0 ?
                                            academicPrograms.map((program, index) => (
                                                <tr className="text-gray-700 dark:text-white" key={index}>
                                                    <td className="p-2 text-center font-bold border-b text-xs md:text-sm" >{++index + (pagination.currentPage - 1) * pagination.perPage}</td>
                                                    <td className="p-2 text-center font-bold border-b text-xs md:text-sm">{program.name}</td>
                                                    <td className="p-2 text-center border-b text-xs md:text-sm ">
                                                        <button className="bg-blue-500 hover:bg-blue-600 text-white py-1 px-2 rounded-lg text-xs mr-2 md:p-3" onClick={(e) => handleEditClick(program, e)}> 
                                                            <i className="fa fa-pencil-alt"></i>
                                                        </button>
                                                        <button onClick={(e) => handleDelete(program, e)} className="bg-red-500 hover:bg-red-600 text-white py-1 px-2 rounded-lg text-xs md:p-3">
                                            ...
                                            <tr className="text-gray-700">
                                                <td className="p-2 text-center font-bold border-b text-xs md:text-sm" colSpan="3">
                                                    No data available.
                                                </td>
                                            </tr>
                                    }
